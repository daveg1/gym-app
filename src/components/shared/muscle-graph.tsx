...
export interface MuscleGraphProps {
  isEnabled: boolean;
  view: "front" | "back";
...
const useTouchHighlight = (enabled = true) => {
  ...
  function handleTouch(touches: TouchList) {
    ...
    if (el?.nodeName.toLowerCase() === "g") {
      newTarget = el;
    } else if (el?.parentElement?.nodeName.toLowerCase() === "g") {
      newTarget = el?.parentElement;
    }

    if (!newTarget || newTarget === dragTargetRef.current) return;
    dragTargetRef.current?.classList.remove(colors.red);

    dragTargetRef.current = newTarget;
    dragTargetRef.current.classList.add(colors.red);
  }
...
export function MuscleGraph(props: MuscleGraphProps) {
  return props.view === "back" ? (
    <BackView {...props} />
  ) : (
    <FrontView {...props} />
  );
...
function FrontView(props: MuscleGraphProps) {
  const [elementRef] = useTouchHighlight(!!props.isEnabled);

  const handleSelect = (group: MuscleGroups) => {
    if (props.isEnabled) {
      props.onMuscleSelect(group);
    }
  };

  return (
    // totally not ripped from https://musclewiki.com/ lol
    <svg
      viewBox="0 0 660.46 1206.46"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="object-fit w-full touch-none px-4"
      ref={elementRef as unknown as React.Ref<SVGSVGElement>}
      onClick={() => props.onClick?.()}
    >
      <g
        id="calves"
        onTouchEnd={() => handleSelect("Calves")}
      ...
      <g
        id="obliques"
        onTouchEnd={() => handleSelect("Obliques")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="forearms"
        onTouchEnd={() => handleSelect("Forearms")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="biceps"
        onTouchEnd={() => handleSelect("Biceps")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="front-shoulders"
        onTouchEnd={() => handleSelect("Shoulders Front")}
        className="text-gray-300"
      >
        <path
...
function BackView(props: MuscleGraphProps) {
  const [elementRef] = useTouchHighlight(!!props.isEnabled);
  ...
  return (
    ...
    >
      ...
      <g
        id="hamstrings"
        onTouchEnd={() => handleSelect("Hamstrings")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="glutes"
        onTouchEnd={() => handleSelect("Glutes")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="triceps"
        onTouchEnd={() => handleSelect("Triceps")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="lowerback"
        onTouchEnd={() => handleSelect("Lowerback")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="traps-middle"
        onTouchEnd={() => handleSelect("Traps Middle")}
        className="text-gray-300"
      >
        <path
      ...
      <g
        id="rear-shoulders"
        onTouchEnd={() => handleSelect("Shoulders Rear")}
        className="text-gray-300"
      >
        <path
